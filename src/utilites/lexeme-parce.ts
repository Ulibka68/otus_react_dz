...
function getOperator(opname: string): operand {
  ...
  // @ts-ignore
  return new operandCl(op.opName, op.priority, op.type, op.exec); // eslint-disable-line
}
...
export function convertToPolishNote(parcesNum: NumStr[]): (NumStr | operand)[] {
      ...
      if (funcStack.length === 0) {
        ...
        if (curOperator === "+" || curOperator === "-") {
          ...
          if (i === 0) {
            ...
            curOperator = curOperator === "+" ? "UNARY_PLUS" : "UNARY_MINUS";
          } else {
            if (parcesNum[i - 1] === "(") {
              // это унарный оператор
              curOperator = curOperator === "+" ? "UNARY_PLUS" : "UNARY_MINUS";
        ...
        if (curOperator === "(") {
          ...
          if (curOperator === ")") {
            ...
            while (
              funcStack.length > 0 &&
              funcStack[funcStack.length - 1].priority >= opObj.priority
            ) {
              operandStack.push(funcStack[funcStack.length - 1]);
              funcStack.pop();
            }
  ...
  if (funcStack.length > 0) {
    ...
    while (funcStack.length > 0) {
      // eslint-disable-next-line
      // @ts-ignore
      operandStack.push(funcStack.pop()); // eslint-disable-line
    }
  }
